...
document.getElementById("mode-filter").addEventListener("change", function () {
    const diffGroup = document.getElementById("difficulty-filter").closest(".filter-group");
    diffGroup.style.display = this.value === "mountain" ? "none" : "";
});
...
async function loadQuestions() {
    ...
    if (mode === "mountain" && !category) {
        alert("Please select a category for Climb the Mountain mode.");
        return;
    }
    ...
    if (mode === "mountain") params.set("limit", "100");
